...
class TestValidContactMock:
    def test_returns_response(self, mock_cur):
        ...
        result = nv.valid_contact(
            cur=mock_cur, yml_dict=yml, csv_file=[], tables=["ndb.datasetpis"]
        )
    ...
    def test_no_contact_info_skips_gracefully(self, mock_cur):
        ...
        result = nv.valid_contact(
            cur=mock_cur, yml_dict={"metadata": []}, csv_file=[], tables=["ndb.datasetpis"]
        )
        assert isinstance(result, Response)
    ...
    def test_all_tables_iterated(self, mock_cur):
        ...
        result = nv.valid_contact(cur=mock_cur, yml_dict=[], csv_file=[], tables=CONTACT_TABLES)
        assert isinstance(result, Response)
...
class TestValidContactInsertDispatch:
    ...
    def _run_with_found_contact(self, mock_cur, table, databus):
        ...
        try:
            ...
            result = nv.valid_contact(
                cur=mock_cur, yml_dict=yml, csv_file=[], tables=[table], databus=databus
            )
        finally:
    ...
    def test_collector_skipped_when_no_collunit(self, mock_cur):
        ...
        result = self._run_with_found_contact(mock_cur, "ndb.collectors", databus)
        msgs = " ".join(result.message)
    ...
    def test_chronologies_message_about_no_separate_insert(self, mock_cur):
        ...
        result = self._run_with_found_contact(mock_cur, "ndb.chronologies", databus)
        msgs = " ".join(result.message)
