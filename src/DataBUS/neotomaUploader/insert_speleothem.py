...
def insert_speleothem(cur, yml_dict, csv_file, uploader):
    ...
    """
    params = ['entityid', 'entityname', 'monitoring', 'rockageid', 'entrancedistance', 
    ...
    sisal_entity_id = inputs.get('entityid')
    ...
    try:
        ...
        sp.insert_entitycovers_to_db(cur, id = spid,
        ...
        sp.insert_entitylandusecovers_to_db(cur, id = spid, 
        ...
        sp.insert_entityvegetationcovers_to_db(cur, id = spid, 
        ...
        with open(file_path, "a", newline="") as f:
            writer = csv.writer(f)
            if write_header:
                writer.writerow(["neotoma_entityid", "sisal_entityid", "entitystatusid", "sisal_reference_id"])
            for i in inputs.get("ref_id", []):
                writer.writerow([spid, sisal_entity_id, inputs.get("entitystatusid"), i])
        response.valid.append(True)
    except Exception as e:
        response.valid.append(False)
