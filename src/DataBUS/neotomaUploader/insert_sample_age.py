...
def insert_sample_age(cur, yml_dict, csv_file, uploader):
    ...
    for chron, chron_id in uploader['chronology'].name.items():
        try:
            sa = inputs['sampleages'][chron]
        except KeyError:
            response.message.append(f"✗ No sample ages found for chronology {chron}.")
            continue
        ...
        for idx, sa_id in enumerate(uploader['samples'].sampleid):
            try:
                ...
                if sa.get('age') is None:
                    response.valid.append(True)
                    response.message.append(f"? No age provided for sample ID {sa_id}.")
                    continue
                else:
                    s = {}
