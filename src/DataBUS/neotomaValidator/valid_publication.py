...
def valid_publication(cur, yml_dict, csv_file):
    ...
    try:
        ...
        if not any(inputs.get(param) for param in params):
            response.message.append("? No publication information provided. Publication info will not be uploaded.")
    ...
    queries = {'doi': """SELECT publicationid FROM ndb.publications
    ...
    for i, pub in enumerate(inputs.get(search_param[0], [])):
        ...
        if result:
            ...
            if search_param[1] == 2:
                ...
                response.message.append(f"✗  The publication does not exist in Neotoma: {inputs[search_param[0]][i]}.")
                response.valid.append(False)
    return response
