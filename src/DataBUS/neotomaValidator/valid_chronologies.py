...
def valid_chronologies(cur, yml_dict, csv_file, databus=None):
    ...
    for chron_key in inputs:
        ...
        if isinstance(ch["contactid"], str):
            ...
            response.message.append(f"✔ Contact resolved: {result['name']} (ID: {result['id']})")
            response.valid.append(True)
        try:
            ...
            response.valid.append(False)
            response.message.append(
                f"✗ Age bounds could not be converted from collection date to years BP: {e}"
            )
            continue
        try:
            ...
            try:
                ...
                response.message.append(
                    f"✔  Chronology inserted with ID {chron_id} ('{chron_key}')."
                )
                response.valid.append(True)
                ...
                response.valid.append(False)
                response.message.append(f"✗  Chronology could not be inserted: {e}")
        except Exception as e:
            response.valid.append(False)
