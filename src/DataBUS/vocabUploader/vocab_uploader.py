...
def vocab_uploader(cur, conn, yml_dict, csv_file, table, upload=False, logfile=None):
    if logfile is None:
        logfile = []
    ...
    inputs = {
        ...
        if v is not None and not (isinstance(v, list) and all(x is None for x in v))
    }
    outputs = []
    if table == "ndb.taxa":
        id_params = {
            "publicationid": """SELECT publicationid
        ...
        for row in zip(*inputs.values(), strict=False):
                ...
                if isinstance(param_value, str) and param_value.strip() != "":
                    try:
                        ...
                        if result is not None:
                            ...
                            logfile.append(
                                f"Warning: '{param_value}' for parameter '{param}' does not exist in the database. It will be treated as NULL."
                            )
                            row[param] = None
            output = row.copy()
            optional_fields = [p for p in ["author"] if p != "taxonname" and row.get(p) is not None]
            conditions = ["taxonname ILIKE %(taxonname)s"] + [
                f"{p} IS NOT DISTINCT FROM %({p})s" for p in optional_fields
            ]
            ...
            if result is not None:
                ...
                logfile.append(
                    f"Taxon '{row['taxonname']}' already exists in the database with taxonid {result[0]}."
                )
            else:
                insert_query = """
            ...
            if upload:
                ...
                conn.rollback()
                logfile.append(
                    f"Validated new taxon '{row['taxonname']}' with taxonid {output.get('taxonid', 'N/A')}. Not uploaded to database."
                )
